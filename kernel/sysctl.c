			  ...
			  struct ctl_table *table)
{
	...
	/* only register a file table for this directory if:
	 * a) there are actual files in the directory
	 * b) there's no child subdir => register an empty dir */
	if ((nr_files != 0) || ((nr_files == 0) && (nr_dirs == 0))) {
		files = kmalloc(sizeof(struct ctl_table) * (nr_files + 1), GFP_KERNEL);
		if (files == NULL)
 ...
 * series of simpler registration that only register tables of files.
 *
 * To maintain full compatibility with users of this function we
 * extend the ctl_table_header to hold information about the
 * subheaders.
 *
